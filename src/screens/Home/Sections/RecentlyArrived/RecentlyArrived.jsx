...
const RecentlyArrived = () => {
  ...
  useEffect(() => {
    const fetchOffersAndWishlist = async () => {
      ...
      try {
        const prodRes = await api.get("/api/products/recently-arrived");
  ...
  // Compute remaining stock & prepare for display
  useEffect(() => {
    if (!offers.length) return;
    const withStockLeft = offers.map((p) => {
      const already =
        cartItems.find((c) => c.productId === p.id)?.quantity || 0;
      return { ...p, stockLeft: Math.max(p.stock - already, 0) };
    });
  ...
  const handleAddToCart = async (productId, quantity = 1, variant = null) => {
    ...
    if (token) {
      ...
      const idx = lsCart.findIndex(
        (c) => c.productId === productId && c.variant === variant
      );
      if (idx > -1) lsCart[idx].quantity += quantity;
      else lsCart.push({ ...cartItem, product: prod });
      ...
      <div className="pt-12 px-12 lg:px-32">
        ...
        <div className="w-full flex items-center justify-between pb-4">
          <h1 className="text-[#1a1713] text-[20px] sm:text-[24px] font-bold [font-family:'Cairo']">
            {isRTL ? "وصل حديثا" : "Recently Arrived"}
          </h1>
          <Button
            variant="ghost"
